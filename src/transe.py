    ...
    def transE(self, cI=20):
        ...
        for cycleIndex in range(cI):
            ...
            for sbatch in Sbatch:
                tripletWithCorruptedTriplet = (sbatch, self.getCorruptedTriplet(sbatch))#正确三元组 + 打乱三元组
            ...
            if cycleIndex % 100 == 0:
                ...
                self.writeRelationVector("../data/result/relationVector.txt")
                self.writeEntilyVector("../data/result/entityVector.txt")
                self.loss = 0
...
if __name__ == '__main__':
    ...
    print("---START TransE---")
    transE = TransE(entityList, relationList, tripleList, margin=1, learingRate = 0.005, dim=75)
    '''
    dim = 50
    ---正确率---  50.29 %
    ---@hits2---  45.4 %
    ...
    ---@hits2---  47.79 %
    '''
    # transE = TransE(entityList, relationList, tripleList, learingRate = 0.0001, dim = 50)
    print("---TranE INIT---")
    transE.initialize()
    transE.transE(1000)
